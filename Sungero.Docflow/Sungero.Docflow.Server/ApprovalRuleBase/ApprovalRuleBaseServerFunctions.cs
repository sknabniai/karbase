...
namespace Sungero.Docflow.Server
{
  partial class ApprovalRuleBaseFunctions
  {
    ...
    /// <param name="signatory">Подписывющй.</param>
                                                            ...
                                                            bool withApprovalRule)
    {
      ...
      var stages = Functions.ApprovalTask.GetStages(task);
      var stageNumber = 1;
      
      if (stages.IsConditionsDefined && withApprovalRule)
      {
        ...
        {
          var expectedDate = task.Status != Docflow.ApprovalTask.Status.Draft ? task.MaxDeadline : Docflow.Functions.ApprovalTask.GetExpectedDate(task, null, stages.Stages);
      ...
      foreach (var ruleStage in stages.Stages)
      {
        ...
        {
          var stageStatusInfo = GetStatusInfo(task, ruleStage, stages.Stages);
          var showControlReturnStage = NeedShowControlReturnStage(task, ruleStage, stageStatusInfo);
        ...
        var statusInfo = GetStatusInfo(task, ruleStage, stages.Stages);
        ...
        AddPerformersToBlock(stageBlock, performersRecipients, stage, emptyPerformersLabel);
      ...
      if (!stages.IsConditionsDefined)
      {
        ...
        if (task.DocumentGroup.OfficialDocuments.Any())
        {
          ...
          errorBlock.AddLabel(stages.ErrorMessage, noteStyle);
        }
        else
        {
    ...
    private static void AddPerformersToBlock(StateBlock block, List<Sungero.CoreEntities.IRecipient> performers, IApprovalStage stage, string emptyPerformersLabel)
    {
      var noteStyle = Functions.Module.CreateNoteStyle();
      var performerStyle = Functions.Module.CreatePerformerDeadlineStyle();
      ...
      {
        // Если исполинтель не первый - добавить перевод строки.
    ...
    private static void PrintPerformer(StateBlock block, IRecipient performer)
    {
      var performerStyle = Functions.Module.CreatePerformerDeadlineStyle();
      var noteStyle = Functions.Module.CreateNoteStyle();
      var groupNameStyle = Functions.Module.CreateStyle(true, true);
    ...
    /// <param name="ruleStage">Определеляемый этап.</param>
    /// <param name="stages">Этапы.</param>
    /// <returns>Информация о состоянии этапа.</returns>
    public static Structures.ApprovalRuleBase.StageStatusInfo GetStatusInfo(IApprovalTask task, Structures.Module.DefinedApprovalStageLite ruleStage, List<Structures.Module.DefinedApprovalStageLite> stages)
    {
      ...
      var collapsedStages = Functions.ApprovalTask.GetCollapsedStages(task, ruleStage);
      if (collapsedStages != null && collapsedStages.Contains(inProcessStage))
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по сумме.
    /// </summary>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по контрагенту-нерезиденту.
    /// </summary>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по подписаннию документа контрагентом.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по виду во вложениях.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по роль-роль.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по роль-сотрудник.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по проектным документам.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по способам доставки.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    private bool CheckDeliveryMethodsConditions(List<Structures.ApprovalRuleBase.RouteStep> allConditions, Structures.ApprovalRuleBase.RouteStep condition)
    {
      ...
      foreach (var previousCondition in allConditions.TakeWhile(x => !Equals(x, condition)))
      {
        ...
        var previousDeliveryMethods = previoisConditionItem.Condition.DeliveryMethods.Select(x => x.DeliveryMethod).ToList();
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по валюте.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    private bool CheckCurrencyConditions(List<Structures.ApprovalRuleBase.RouteStep> allConditions, Structures.ApprovalRuleBase.RouteStep condition)
    {
      ...
      {
        var previoisConditionItem = _obj.Conditions.Where(x => x.Number == previousCondition.StepNumber).FirstOrDefault();
        var previousCurrencies = previoisConditionItem.Condition.Currencies.Select(x => x.Currency).ToList();
    ...
    /// <summary>
    /// Проверить возможность сущеcтвования данного маршрута с условиями по виду документа.
    /// </summary>
    /// <param name="allConditions">Все условия в данном маршруте.</param>
    /// <param name="condition">Текущее условие.</param>
    /// <returns>Возможность существования данного маршрута.</returns>
    private bool CheckDocumentKindConditions(List<Structures.ApprovalRuleBase.RouteStep> allConditions, Structures.ApprovalRuleBase.RouteStep condition)
    {
      ...
      {
        var previoisConditionItem = _obj.Conditions.Where(x => x.Number == previousCondition.StepNumber).FirstOrDefault();
        var previousDocumentKinds = previoisConditionItem.Condition.ConditionDocumentKinds.Select(x => x.DocumentKind).ToList();
    ...
    public List<Structures.ApprovalRuleBase.StagesIncorrectRoles> CheckImpossibleRoles(List<int> route, bool showErrors, List<Enumeration?> verifiedRoles)
    {
      ...
      var roles = new Dictionary<Enumeration?, Enumeration?>();
      
      roles.Add(Docflow.ApprovalRoleBase.Type.AddrAssistant, Docflow.ApprovalStage.StageType.Review);
      roles.Add(Docflow.ApprovalRoleBase.Type.Addressee, Docflow.ApprovalStage.StageType.Review);
      roles.Add(Docflow.ApprovalRoleBase.Type.SignAssistant, Docflow.ApprovalStage.StageType.Sign);
      roles.Add(Docflow.ApprovalRoleBase.Type.Signatory, Docflow.ApprovalStage.StageType.Sign);
      
      if (verifiedRoles.Any())
        roles = roles.Where(r => verifiedRoles.Contains(r.Key)).ToDictionary(d => d.Key, d => d.Value);
      
      foreach (var role in roles)
      {
        // Этапы, где исполнителем указана роль.
        var stages = _obj.Stages.Where(s => route.Any(r => r == s.Number))
          .Where(s => (s.Stage.ApprovalRole != null && Equals(s.Stage.ApprovalRole.Type, role.Key)) ||
                 s.Stage.ApprovalRoles.Any(x => Equals(x.ApprovalRole.Type, role.Key)));
        
        // Если нет соответствующего этапа.
        if (stages.Any() && !_obj.Stages.Where(s => route.Any(r => r == s.Number)).Any(x => Equals(x.Stage.StageType, role.Value)))
        {
          if (showErrors)
          {
            var message = ApprovalRuleBases.Resources.CantUseRoleWithoutStageFormat(ApprovalRoleBases.Info.Properties.Type.GetLocalizedValue(role.Key),
                                                                                    ApprovalStages.Info.Properties.StageType.GetLocalizedValue(role.Value));
            foreach (var stage in stages)
              incorrectStagesRoles.Add(Structures.ApprovalRuleBase.StagesIncorrectRoles.Create(stage, message));
          }
        }
      }
        ...
        for (int i = 0; i < route.Count - 1; i++)
        {
          ...
          if (transition != null)
          {
            ...
            if (condition != null && condition.Condition.ConditionType == Docflow.ConditionBase.ConditionType.ProjectDocument)
            {
              ...
              {
                var stage = _obj.Stages.SingleOrDefault(s => s.Number == route[j]);
                if (stage == null)
                  continue;
    ...
    private static bool NeedShowSendingStage(IApprovalTask task, Structures.Module.DefinedApprovalStageLite stage, List<Structures.Module.DefinedApprovalStageLite> stages)
    {
      ...
      var lastReapproveAssignment = GetForReapprovingAssignmentResultId(task.Id);
      var assignments = Assignments.GetAll().Where(a => a.Task.Id == task.Id);
      if (lastReapproveAssignment != 0)
        assignments = assignments.Where(x => x.Id > lastReapproveAssignment);
    ...
    private static bool NeedShowControlReturnStage(IApprovalTask task, Structures.Module.DefinedApprovalStageLite stage, Structures.ApprovalRuleBase.StageStatusInfo statusInfo)
    {
      ...
      var lastReworkAssignmentIndex = assignmentsByTask.FindIndex(a => a.Id == lastReapproveAssignment);
    ...
    /// <param name="previoisConditionTypeItems">Дочерняя коллекция предыдущего условия.</param>
    ...
    protected static bool? CheckConsistencyConditions(IEnumerable<IEntity> conditionTypeItems,
                                                      IEnumerable<IEntity> previoisConditionTypeItems,
                                                      Structures.ApprovalRuleBase.RouteStep condition,
                                                      Structures.ApprovalRuleBase.RouteStep previousCondition)
    {
      var currentExceptPrevious = conditionTypeItems.Except(previoisConditionTypeItems);
      var previousExceptCurrent = previoisConditionTypeItems.Except(conditionTypeItems);
      ...
      if (currentExceptPrevious.Count() == conditionTypeItems.Count() && previousExceptCurrent.Count() == previoisConditionTypeItems.Count() && previousCondition.Branch)
        return !condition.Branch;

      // Если предыдущее условие содержит все элементы текущего (и ветка Нет), то возможна только ветка Нет.
      if (!currentExceptPrevious.Any() && previousExceptCurrent.Count() < previoisConditionTypeItems.Count() && !previousCondition.Branch)
        return !condition.Branch;
    ...
    public virtual Structures.Module.DefinedApprovalStages GetStages(IOfficialDocument document, IApprovalTask task)
    {
      ...
      if (document == null)
        return Structures.Module.DefinedApprovalStages.Create(stages, canDefineConditions, string.Empty);
      
      if (_obj.Stages.Count == 1 && !_obj.Transitions.Any())
        return Structures.Module.DefinedApprovalStages.Create(_obj.Stages.Select(s => Structures.Module.DefinedApprovalStageLite.Create(s.Stage, s.Number, s.StageType)).ToList(), true, string.Empty);
      ...
      if (_obj.Stages.Any(x => x.Number == currentStageNumber))
      {
        ...
        stages.Add(Structures.Module.DefinedApprovalStageLite.Create(stage.Stage, stage.Number, stage.StageType));
    ...
    public virtual void CheckRuleStages(Structures.ApprovalRuleBase.StagesVariants stagesVariants, Sungero.Domain.BeforeSaveEventArgs e)
    {
      ...
      foreach (var stepsNumbers in stagesVariants.AllSteps)
      {
        ...
        // Этап расмотрения должен быть после "последнего" этапа подписания.
