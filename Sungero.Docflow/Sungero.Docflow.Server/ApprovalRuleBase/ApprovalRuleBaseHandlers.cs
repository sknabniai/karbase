    ...
    public override void BeforeSave(Sungero.Domain.BeforeSaveEventArgs e)
    {
      ...
      if (_obj.Conditions.Any())
      {
        foreach (var condition in _obj.Conditions)
        {
          ...
          {
            var availableDocumentKinds = Functions.ApprovalRuleBase.GetAvaliableDocumentKinds(_obj);
      ...
      if (_obj.Stages.Any())
      {
        var roles = _obj.Stages.Select(s => s.Stage.ApprovalRole).Where(r => r != null).ToList();
        roles.AddRange(_obj.Stages.SelectMany(s => s.Stage.ApprovalRoles.Select(r => r.ApprovalRole)));
        ...
        foreach (var role in roles)
        {
          if (!Functions.ApprovalRoleBase.SupportDocumentKinds(role, documentKinds))
          {
            ...
            var stagesWithProjectRole = _obj.Stages.Where(s => Functions.ApprovalStage.HasRole(s.Stage, role.Type)).ToList();
