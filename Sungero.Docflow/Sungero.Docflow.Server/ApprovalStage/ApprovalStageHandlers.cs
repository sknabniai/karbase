...
namespace Sungero.Docflow
{
  ...
  partial class ApprovalStageServerHandlers
  {
    public override void BeforeSave(Sungero.Domain.BeforeSaveEventArgs e)
    {
      ...
      // Срок должен быть у всех этапов, кроме уведомления.
      if (!_obj.DeadlineInDays.HasValue && !_obj.DeadlineInHours.HasValue && !isNotice)
      {
      ...
      // Общий срок этапа должен быть больше нуля, если это не уведомление.
      if ((_obj.DeadlineInDays ?? 0) + (_obj.DeadlineInHours ?? 0) == 0 && !isNotice && e.IsValid)
      {
