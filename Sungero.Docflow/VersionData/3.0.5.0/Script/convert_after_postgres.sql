  ...
  SELECT id INTO deliveryMethod
  FROM Sungero_Docflow_DeliveryMethod 
  WHERE name = 
   CASE WHEN russianLocale = '1' THEN 'Сервис эл. обмена' ELSE 'Exchange service' END;
  ...
  newId := (SELECT Sungero_System_GetNewId('Sungero_Docflow_CondBaseDelive', recordsCount));
  ...
  SET 
    Name = CASE WHEN russianLocale = '1' THEN 'Способ доставки - Сервис эл. обмена?' ELSE 'Is delivery method Exchange service?' END,
	  NegationName = CASE WHEN russianLocale = '1' THEN 'Способом доставки не является Сервис эл. обмена?' ELSE 'Is delivery method other than Exchange service?' END,
