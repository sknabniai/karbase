...
namespace Sungero.RecordManagement.Server
{
  partial class AcquaintanceTaskFunctions
  {
    ...
    {
      var performers = GetNonSystemActivePerformers(recipients);
      var excludedPerformers = GetNonSystemActivePerformers(excludedRecipients);
    ...
    {
      var activeRecipients = recipients.Where(x => x != null && x.Status == CoreEntities.DatabookEntry.Status.Active).ToList();
      var performers = Functions.Module.GetAcquaintanceTaskPerformers(activeRecipients)
