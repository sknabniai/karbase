...
namespace Sungero.Parties.Server
{
  partial class CounterpartyFunctions
  {
    ...
    public Structures.Counterparty.SendInvitation InvitationBoxes()
    {
      ...
      result.Services = exchangeServices.OrderByDescending(x => Equals(x.ExchangeProvider, ExchangeCore.ExchangeService.ExchangeProvider.Synerdocs)).ThenBy(x => x.Name).ToList();
