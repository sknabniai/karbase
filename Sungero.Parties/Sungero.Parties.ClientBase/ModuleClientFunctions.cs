        ...
        (args) =>
        {
          ...
          if (Equals(args.Button, nextButton))
          {
            ...
            var counterpartiesFromString = foundFromExchangeServiceCompanies
              .OrderBy(a => a.Box.ExchangeService.ExchangeProvider == ExchangeCore.ExchangeService.ExchangeProvider.Synerdocs)
                                                                            ...
                                                                            Sungero.ExchangeCore.IBusinessUnitBox[] connectedBoxes)
    {
      ...
      if (usersBusinessUnit != null)
        connectedBoxes = connectedBoxes
          .OrderByDescending(b => Equals(b.ExchangeService.ExchangeProvider, ExchangeCore.ExchangeService.ExchangeProvider.Synerdocs))
          .ThenBy(b => !Equals(b.BusinessUnit, usersBusinessUnit))
          .ThenBy(b => b.Name)
          .ToArray();
      else
        connectedBoxes = connectedBoxes
