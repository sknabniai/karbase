...
namespace Sungero.Parties
{
  partial class CounterpartyClientHandlers
  {
    ...
    {
      var result = Functions.Counterparty.CheckAccountLength(e.NewValue);
    ...
    {
      var result = Functions.Counterparty.CheckNceoLength(_obj, e.NewValue);
      if (!string.IsNullOrEmpty(result))
        e.AddError(result);
    }
    ...
    {
      var result = Functions.Counterparty.CheckPsrnLength(_obj, e.NewValue);
      if (!string.IsNullOrEmpty(result))
        e.AddError(result);
    }
    ...
    public virtual void TINValueInput(Sungero.Presentation.StringValueInputEventArgs e)
    {
      ...
      {
        var result = Functions.Counterparty.CheckTin(_obj, e.NewValue);
        if (!string.IsNullOrEmpty(result))
          e.AddError(_obj.Info.Properties.TIN, result, _obj.Info.Properties.Nonresident);
      }
    }
